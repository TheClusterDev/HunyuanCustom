...
def create_demo():
    with gr.Blocks() as demo:
        ...
        with gr.Tab('Audio-Driven Video Customization'):
            with gr.Row():
                with gr.Column(scale=1):
                    with gr.Group():
                        prompt = gr.Textbox(label="Prompt", value="")
                        neg_prompt = gr.Textbox(label="Negative Prompt", value="")
                        audio_strength = gr.Slider(0.0, 1.0, 0.8, step=0.1, label="Audio Strength")
                    id_image = gr.Image(label="Input reference image", height=480)
                    audio_input = gr.Audio(type="filepath", label="Upload Audio", elem_classes="media-upload", scale=1)
            ...
            with gr.Row():
                with gr.Column(scale=2):
                    with gr.Accordion("Options for generate video", open=False):
                        ...
                        with gr.Row():
                            num_steps = gr.Slider(1, 100, 50, step=5, label="Number of steps")
                        ...
                        with gr.Row():
                            ...
                            template_neg_prompt = gr.Textbox(label="Template Negative Prompt", value="Two people, two persons, aerial view, " \
            ...
            generate_btn.click(fn=post_and_get,
                inputs=[id_image, audio_input, audio_strength, width, height, num_frames, num_steps, flow_shift, guidance, seed, prompt, neg_prompt, template_prompt, template_neg_prompt, name],
            ...
            with gr.Row(), gr.Column():
                ...
                example_inps = [
                    [
                        'In a bakery, a woman holds a cake, introducing it.',
                        'assets/images/seg_woman_01.png',
                        'assets/audios/cake_woman.mp3',
                        0.8, 1280, 720, 30, 129, 7.5, 13, 1024,        
                        "assets/videos/seg_woman_01_audio.mp4"
                    ], 
                    [
                        'A woman is speaking in a room.',
                        'assets/images/seg_woman_04.png',
                        'assets/audios/greek_woman.mp3',
                        0.8, 1280, 720, 30, 129, 7.5, 13, 1024,        
                        "assets/videos/seg_woman_04_audio.mp4"
                    ], 
                    [
                        'In the study, a man sits at a table featuring a bottle of milk while delivering a product presentation.',
                        'assets/images/seg_man_01.png',
                        'assets/audios/milk_man.mp3',
                        0.8, 1280, 720, 30, 129, 7.5, 13, 1026,
                        "assets/videos/seg_man_01_audio.mp4"
                    ], 
                    [
                        'A person is sitting on a sofa and talking.',
                        'assets/images/seg_man_04.png',
                        'assets/audios/milk_man.mp3',
                        0.8, 1280, 720, 30, 129, 7.5, 13, 1027,
                        "assets/videos/seg_man_04_audio.mp4"
                    ], 
                ]
                gr.Examples(examples=example_inps, inputs=[prompt, id_image, audio_input, audio_strength, width, height, num_steps, num_frames, guidance, flow_shift, seed, output_image],)
